  ...
  return (
    <div className="fixed inset-x-0 bottom-0 top-14 flex min-h-0 flex-col overflow-hidden px-5 py-5">
      ...
      <div className="mx-auto mt-4 w-full max-w-7xl shrink-0">
        {error && <p className="mb-3 rounded-lg border border-destructive/25 bg-destructive/10 px-3 py-2 text-sm text-destructive">{error}</p>}
        {message && <p className="mb-3 rounded-lg border border-primary/25 bg-primary/10 px-3 py-2 text-sm text-primary">{message}</p>}
      </div>
          ...
          <div className="mb-3 shrink-0 rounded-lg border border-border bg-card p-4">
            ...
            {activeTab === "items" ? (
              ...
              <div className="flex flex-wrap items-center gap-2 text-sm">
                <span className="text-muted-foreground">当前 dataset</span>
            ...
            {currentSpace && (
              <div className="mt-3 grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                ...
                <input
                  ...
                  placeholder="Dify dataset id，留空则跳过同步"
                  className="rounded-lg border border-border bg-background px-3 py-2 text-sm"
                />
                <button
                  onClick={saveSpace}
                ...
                <textarea
                  ...
                  placeholder="知识库说明"
                  rows={2}
                  className="resize-y rounded-lg border border-border bg-background px-3 py-2 text-sm md:col-span-3"
                />
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-background">
          <div className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-border px-5">
            <button
              ...
              className="shrink-0 rounded-lg border border-primary bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground hover:bg-primary-hover"
            >
              返回知识库
            </button>
            ...
            <button
              onClick={() => setPreviewExpanded(false)}
              className="shrink-0 rounded-lg border border-border px-3 py-1.5 text-sm hover:bg-muted"
            >
              关闭
            </button>
          </div>
          ...
          <button
            onClick={() => setPreviewExpanded(false)}
            className="fixed bottom-5 left-5 z-[60] rounded-lg border border-primary bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-lg hover:bg-primary-hover"
          >
            返回知识库
          </button>
        </div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="mb-2 flex flex-wrap gap-1.5 text-xs">
                  <span className="rounded-full bg-muted px-2 py-0.5 text-muted-foreground">{item.item_kind}</span>
                  <span className="rounded-full bg-muted px-2 py-0.5 text-muted-foreground">{item.source_type || "unknown"}</span>
                  <span className="rounded-full bg-muted px-2 py-0.5 text-muted-foreground">{item.sync_status}</span>
                  {item.run_mode && <span className="rounded-full bg-muted px-2 py-0.5 text-muted-foreground">{item.run_mode}</span>}
                  {item.run_status && <span className="rounded-full bg-muted px-2 py-0.5 text-muted-foreground">{item.run_status}</span>}
                </div>
                <h2 className="break-words text-base font-semibold">{itemTitle(item)}</h2>
                <p className="mt-1 break-all text-xs text-muted-foreground">
                  item: {item.item_id}
                  {item.paper_id && `；paper: ${item.paper_id}`}
              ...
              <div className="flex flex-wrap gap-2">
                {item.paper_id && (
                  <Link href={`/papers#paper-${item.paper_id}`} className="rounded-lg border border-border px-2.5 py-1.5 text-xs hover:bg-muted">
                    论文
                  </Link>
                )}
                {item.paper_id && item.run_id && (
                  <Link href={`/paper/${item.paper_id}/run/${item.run_id}`} className="rounded-lg border border-border px-2.5 py-1.5 text-xs hover:bg-muted">
                    解读
                  </Link>
                )}
                <button
                ...
                <button
                  ...
                  disabled={Boolean(acting)}
                  className="rounded-lg border border-destructive/30 px-2.5 py-1.5 text-xs text-destructive hover:bg-destructive/10 disabled:opacity-50"
                >
    ...
    return (
      <div className="min-h-0 flex-1 rounded-lg border border-border bg-card p-5">
        <p className="text-sm text-muted-foreground">
          当前知识库尚未绑定 Dify dataset。绑定后，本地论文原文和解读才能同步到对应的 Dify 知识库。
        </p>
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button
            ...
            disabled={Boolean(acting)}
            className="rounded-lg border border-primary bg-primary px-3 py-2 text-sm text-primary-foreground hover:bg-primary-hover disabled:opacity-50"
          >
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="mt-1 block truncate text-xs text-muted-foreground">
                        {doc.indexing_status || doc.display_status || "unknown"}
                        {typeof doc.word_count === "number" ? ` · ${doc.word_count} words` : ""}
                      </span>

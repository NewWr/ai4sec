...
async def step_4_expand_graph_candidates(
    ...
    try:
        ...
        _cfg = get_settings()
        _llm_cfg = get_llm_runtime_config()
...
async def step_7_layer1_abstract_snap(
    ...
    async def _analyze_batch(
        ...
        try:
            response = await llm.chat(messages, model=model, temperature=0.2)
...
async def step_9_synthesize_landscape(
    ...
    async def _run_classifier() -> dict[str, Any] | None:
        try:
            ...
            classifier_resp = await llm.chat(classifier_messages, model=model, temperature=0.2)
    ...
    async def _run_comparator() -> dict[str, Any] | None:
        try:
            ...
            comparator_resp = await llm.chat(comparator_messages, model=model, temperature=0.2)
    ...
    try:
        ...
        advisor_resp = await llm.chat(advisor_messages, model=model, temperature=0.3)

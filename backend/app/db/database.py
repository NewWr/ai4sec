...
async def _ensure_external_paper_note_tables(conn: aiosqlite.Connection) -> None:
    await conn.executescript(
        """
...
async def close_db() -> None:
    ...
    global _conn, _read_pool, _read_pool_next, _write_lock
...
async def init_db() -> None:
    ...
    async with aiosqlite.connect(path) as db:
        ...
        try:
            await _ensure_external_paper_note_tables(db)
            await db.commit()
        except Exception:
            logger.exception("Failed to create external paper note tables")

...
async def _unpaywall_find_pdf_url(
    ...
    *,
    ...
    if not doi or not email:
        return None
    api_url = (get_settings().unpaywall_api_url or "").strip()
    if not api_url:
        return None

    try:
        resp = await client.get(
            api_url.format(doi=doi),
            params={"email": email},
            timeout=timeout,
...
async def _core_find_oa_url(
    ...
    *,
    ...
    if not query:
        return None
    api_url = (get_settings().core_search_api_url or "").strip()
    if not api_url:
        return None
...
async def download_via_elsevier_tdm(
    ...
    *,
    ...
    api_url = (get_settings().elsevier_article_api_url or "").strip()
    if not api_url:
        return DownloadResult(
            doi=doi,
            ok=False,
            source="elsevier_tdm",
            pdf_path=None,
            detail="ELSEVIER_ARTICLE_API_URL is not configured",
        )
...
async def download_via_wiley_tdm(
    ...
    *,
    ...
    api_url = (get_settings().wiley_tdm_api_url or "").strip()
    if not api_url:
        return DownloadResult(
            doi=doi,
            ok=False,
            source="wiley_tdm",
            pdf_path=None,
            detail="WILEY_TDM_API_URL is not configured",
        )

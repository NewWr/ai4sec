...
class QueryIntent:
    ...
    preferred_parts: tuple[str, ...] = ("main_body",)
...
def _detect_intent(question: str, tokens: list[str]) -> QueryIntent:
    ...
    if any(term in q_l for term in ("补充", "附录", "消融", "超参", "证明", "复现", "实现细节")) or (
        token_set & {"supplementary", "supplemental", "appendix", "ablation", "hyperparameter", "proof", "reproducibility"}
    ):
        return QueryIntent(
            name="supplementary",
            prefer_tables=True,
            ...
            section_terms=(
                "supplementary", "supplemental", "appendix", "ablation", "implementation",
                "hyperparameter", "proof", "additional", "extended", "补充", "附录", "消融", "证明",
            ),
            preferred_parts=("supplementary", "appendix", "main_body"),
        )
    if any(term in q_l for term in ("指标", "数据", "性能", "实验", "结果", "准确率", "鲁棒", "保真")) or (
        ...
        return QueryIntent(
            ...
            preferred_parts=("main_body", "supplementary", "appendix"),
        )
    if any(term in q_l for term in ("方法", "方案", "算法", "模型", "框架", "架构")) or (
        ...
        return QueryIntent(
            ...
            preferred_parts=("main_body", "appendix", "supplementary"),
        )
    ...
    for block in blocks:
        ...
        nodes.append(
            PaperNode(
                ...
                doc_part=block_doc_part(block, partitions),
            )
        )
...
def _score_node(
    ...
    if intent.name == "summary" and node.block_type in {"title", "text"} and node.page_start <= 1:
        score += 1
    if node.doc_part in intent.preferred_parts:
        preferred_rank = intent.preferred_parts.index(node.doc_part)
        score += max(0, 4 - preferred_rank * 2)
    elif node.doc_part in {"references", "unknown_tail"}:
        score -= 3
...
def _format_context(nodes: list[PaperNode], max_chars: int) -> str:
    ...
    for node in nodes:
        ...
        doc_part = f" [{node.doc_part}]" if node.doc_part else ""
        parts.append(f"[p.{node.page_start + 1}]{doc_part}{section} {text}")
...
def _assign_doc_parts(nodes: list[PaperNode], partitions: list[DocumentPartition]) -> list[PaperNode]:
    if not partitions:
        return nodes
    assigned: list[PaperNode] = []
    for node in nodes:
        part = node.doc_part
        if node.node_type == "chunk":
            matches = [
                partition for partition in partitions
                if partition.block_start <= node.block_start <= partition.block_end
            ]
            if matches:
                matches.sort(key=lambda p: (p.block_end - p.block_start, -p.confidence))
                part = matches[0].part
        assigned.append(PaperNode(**{**node.__dict__, "doc_part": part}))
    return assigned

...
class Settings:
    ...
    semantic_api_base: str = "https://api.semanticscholar.org"

    # IEEE Xplore
    ieee_api_key: str = ""
    ieee_api_url: str = "https://ieeexploreapi.ieee.org/api/v1/search/articles"
    ...
    def from_env() -> "Settings":
        ...
        return Settings(
            ...
            semantic_api_base=(
                os.getenv("PAPERSEARCH_SEMANTICSCHOLAR_API_BASE")
            ...
            ieee_api_url=(
                os.getenv("PAPERSEARCH_IEEE_API_URL")
                or "https://ieeexploreapi.ieee.org/api/v1/search/articles"
            ).strip(),
            ieee_per_second_limit=max(int(ieee_per_second_limit), 1),
            ieee_daily_limit=max(int(ieee_daily_limit), 1),
            openalex_api_base=(
                os.getenv("PAPERSEARCH_OPENALEX_API_BASE")
                or os.getenv("OPENALEX_API_BASE")
                or "https://api.openalex.org"
            ).strip().rstrip("/"),
            crossref_api_base=(
                os.getenv("PAPERSEARCH_CROSSREF_API_BASE")
                or os.getenv("CROSSREF_API_BASE")
                or "https://api.crossref.org"
            ).strip().rstrip("/"),

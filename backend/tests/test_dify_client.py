...
class DifyClientTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_search_payload_and_short_path(self) -> None:
        ...
        self.assertEqual(
            calls["json"],
            {"query": "badclip", "top_k": 5, "search_method": "keyword_search"},
        )
    ...
    async def test_search_defaults_method_from_settings(self) -> None:
        ...
        self.assertEqual(calls["json"]["search_method"], "keyword_search")
    ...
    async def test_create_document_by_text_uses_proxy_path(self) -> None:
        calls, result = await self._run(
            lambda: dify_client.create_document_by_text("Paper", "Body text"),
            settings=_settings(dataset_id="DS123"),
            response=httpx.Response(200, json={"document": {"id": "doc123"}}),
        )
        self.assertEqual(calls["method"], "POST")
        self.assertTrue(calls["url"].endswith("/api/datasets/DS123/documents/create-by-text"), calls["url"])
        self.assertEqual(
            calls["json"],
            {
                "name": "Paper",
        ...
        self.assertEqual(dify_client.extract_document_id(result), "doc123")

    async def test_create_dataset_uses_proxy_path(self) -> None:
        calls, result = await self._run(
            lambda: dify_client.create_dataset("Analysis KB"),
            settings=_settings(),
            response=httpx.Response(200, json={"id": "dataset123", "name": "Analysis KB"}),
        )
        self.assertEqual(calls["method"], "POST")
        self.assertTrue(calls["url"].endswith("/api/datasets"), calls["url"])
        self.assertEqual(
            calls["json"],
            {
                "name": "Analysis KB",
        ...
        self.assertEqual(result["id"], "dataset123")
